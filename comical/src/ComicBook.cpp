...
wxBitmap * ComicBook::GetPage(uint pagenumber)
{
	...
	while (!Resamples[pagenumber].Ok())
		wxUsleep(10); // is this a good number (or a good idea?)
...
{
	uint i;
	int low, high, target, curr;

	wxLogVerbose("ComicBook thread started.");
	while (!TestDestroy()) {
		curr = int(current); // in case this value changes midloop
